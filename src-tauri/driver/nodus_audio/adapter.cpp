...
// ── Endpoint Default-Format seeding (t10) ────────────────────────────────────
// A dynamic subdevice registers its device interface at runtime under a
// reference name the INF can't target. From the kernel (SYSTEM) we seed the EP\0
// Default Format = 48000/2/16 into that interface's registry storage key — else
// audiodg may read our stereo ring as a persisted mono default (2:1 decimation =
// "orc", t10). Per-endpoint NAMES are handled in userspace via the
// AudioEndpointBuilder broker (PKEY_Device_DeviceDesc); kernel interface-property
// seeds don't reach the endpoint name (confirmed on 1.0.51).
//
...
// Seed a dynamic endpoint's Default Format on its wave interface. Best-effort:
// any miss only leaves the prior default, never fails the install.
static VOID NodusSeedEndpoint(_In_ PCWSTR WaveRef, _In_ BOOLEAN Capture)
{
    PAGED_CODE();

    const GUID* waveCat = Capture ? &KSCATEGORY_CAPTURE : &KSCATEGORY_RENDER;
    PWSTR list = nullptr;
    NTSTATUS status = IoGetDeviceInterfaces(waveCat, g_NodusAdapter.Pdo,
    ...
    const size_t refLen = wcslen(WaveRef);
    for (PWSTR link = list; *link; link += wcslen(link) + 1) {
        ...
        if (tail[-1] != L'\\' || _wcsicmp(tail, WaveRef) != 0) continue;

        UNICODE_STRING symlink;
        RtlInitUnicodeString(&symlink, link);
        HANDLE hIf = nullptr;
        NTSTATUS s = IoOpenDeviceInterfaceRegistryKey(&symlink, KEY_ALL_ACCESS, &hIf);
        if (NT_SUCCESS(s)) {
            s = NodusWriteFormat(hIf);
            ZwClose(hIf);
        }
...
NTSTATUS NodusInstallDynamicDevice(ULONG Id, ULONG Kind, PCWSTR FriendlyName)
{
    ...
    // Seed the endpoint's Default Format (stereo, kills "orc"). The per-endpoint
    // name is set in userspace via the broker. Best-effort.
    NodusSeedEndpoint(slot->WaveName, capture);
