/* canvas.jsx — infinite canvas, edges, ports, node component (v2). */

const { useRef, useState, useEffect, useCallback } = React;

const NODE_W = 200, NODE_W_COMPACT = 178;
...
/* stop node drag when interacting with a control inside the body */
...
function NodusNode(props) {
  ...
  // volume meter (input / gain)
  const volMeter = m.vol ? (() => {
    const v = node.volume ?? 80;
    const pct = live && !node.muted && active ? Math.max(3, Math.round(v * (0.55 + level * 0.45))) : 0;
  ...
  // per-edge volume rows: mixer/output show per-INPUT, splitter shows per-OUTPUT
  ...
  // dynamic "+" ports — drag a wire TO an input "+" to add+connect an input;
  // drag FROM an output "+" to pull a new output and connect it to an input.
  const addSlot = (side) => {
  ...
  // grow the node so absolutely-positioned ports never overflow its body
...
function NodusCanvas(props) {
  ...
  const onVpDown = (e) => {
    ...
    const up = (ev) => {
      ...
      if (maxX - minX < 4 && maxY - minY < 4) { setMarquee(null); return; } // a click, not a drag
      const ids = [];
  ...
  // start a connection FROM an output "+" (the new output port is created on drop)
  ...
  const gridStyle = (() => {
    ...
    const ox = ((view.x % (40 * z)) + 40 * z) % (40 * z), oy = ((view.y % (40 * z)) + 40 * z) % (40 * z);
    if (tweaks.grid === 'dots') return { backgroundImage: `radial-gradient(var(--grid) ${Math.max(1, z)}px, transparent ${Math.max(1, z)}px)`, backgroundSize: `${40 * z}px ${40 * z}px`, backgroundPosition: `${ox}px ${oy}px` };
    return { backgroundImage: `linear-gradient(var(--grid) 1px, transparent 1px), linear-gradient(90deg, var(--grid) 1px, transparent 1px)`, backgroundSize: `${40 * z}px ${40 * z}px`, backgroundPosition: `${ox}px ${oy}px` };
  })();
  const majorStyle = (() => {
    if (tweaks.grid === 'off') return { display: 'none' };
    const z = view.zoom;
    const ox = ((view.x % (200 * z)) + 200 * z) % (200 * z), oy = ((view.y % (200 * z)) + 200 * z) % (200 * z);
    return { backgroundImage: `linear-gradient(rgba(255,255,255,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(255,255,255,0.04) 1px, transparent 1px)`, backgroundSize: `${200 * z}px ${200 * z}px`, backgroundPosition: `${ox}px ${oy}px` };
  })();
  ...
  return h('div', { ref: vpRef, className: 'canvas-vp' + (pending ? ' connecting' : ''), onMouseDown: onVpDown, onWheel, onDragOver, onDrop }, [
    ...
    h('div', { key: 'world', className: 'canvas-world', style: { transform: `translate(${view.x}px, ${view.y}px) scale(${view.zoom})` } }, [
...
Object.assign(window, { NodusCanvas, portAnchor, edgePath, NODE_W, NODE_W_COMPACT, NodusNodeIcons: { xIcon, plusIcon } });
