/* panels.jsx — Toolbar, Library, Inspector, StatusBar, BottomDock, rails. Converted to ES module. */
import React from 'react';
import { Icons, meta, channelLabel, NODE_TEMPLATES, NODE_CATEGORIES, VIRTUAL_TEMPLATES } from './graph-data.jsx';
...
/* ============================ TOOLBAR ============================ */
export function Toolbar({ live, onToggleLive, scenes, activeId, onSwitchScene, onNewScene, onCloseScene, onRenameScene, t, setTweak, onImport, onExport }) {
...
export function Library({ onCollapse, inputDevices = [], outputDevices = [], virtualDevices = VIRTUAL_TEMPLATES, onRescan }) {
  ...
  const rescan = async () => {
    setScanning(true);
    if (onRescan) {
      await onRescan().catch(console.error);
    } else {
  ...
  const inputs = inputDevices.filter(d => !ql || d.name.toLowerCase().includes(ql));
  const outputs = outputDevices.filter(d => !ql || d.name.toLowerCase().includes(ql));
  const virtuals = virtualDevices.filter(d => !ql || d.name.toLowerCase().includes(ql));
...
/* ============================ BOTTOM DOCK ============================ */
export function BottomDock({ open, onToggle, pinned, nodes, edges, allNodes, onUnpin, onPin, onFocus, onEdgeVol, onVolume, onMute, onSolo }) {
  const [q, setQ] = _uS('');
...
/* ============================ STATUS BAR ============================ */
export function StatusBar({ view, setView, onFit, nodeCount, edgeCount, live, activeCount, pttActive }) {
...
/* ============================ COLLAPSE RAILS ============================ */
export function CollapseRail({ side, label, onExpand }) {
...
/* ============================ VIRTUAL DEVICE SETUP DIALOG ============================ */
// Shown on startup when no virtual audio device is found.
// Props: show, status, installing, error, onInstallVbcable, onSkip
export function VirtualDeviceSetupDialog({ show, status, installing, error, onInstallVbcable, onSkip }) {
  if (!show) return null;

  const kind = status?.kind ?? 'not_found';
  const isVbAudio = kind === 'vb_audio';
  const isNodus  = kind === 'nodus_driver';
  const found    = isVbAudio || isNodus;

  const optionCard = (key, icon, title, sub, badge, action, actionLabel, disabled) =>
    h('div', { key, className: 'set-row', style: { display: 'flex', flexDirection: 'column', gap: 10, padding: '14px 16px' } }, [
      h('div', { key: 'top', style: { display: 'flex', alignItems: 'center', gap: 10 } }, [
        h('span', { key: 'ic', style: { fontSize: 20 } }, icon),
        h('div', { key: 'txt', style: { flex: 1 } }, [
          h('div', { key: 't', style: { fontFamily: 'var(--font-mono)', fontSize: 10, letterSpacing: '0.06em', color: 'var(--text)', textTransform: 'uppercase' } }, title),
          h('div', { key: 's', style: { fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--text-dim)', marginTop: 2 } }, sub),
        ]),
        badge ? h('span', { key: 'b', style: { fontFamily: 'var(--font-mono)', fontSize: 9, letterSpacing: '0.08em', padding: '2px 7px', borderRadius: 4, background: 'var(--accent-soft)', color: 'var(--accent)', border: '1px solid var(--accent-line)' } }, badge) : null,
      ]),
      action
        ? h('button', { key: 'btn', className: 'insp-btn' + (disabled ? '' : ''), disabled, onClick: action, style: { opacity: disabled ? 0.5 : 1, cursor: disabled ? 'not-allowed' : 'pointer' } },
  ...
  return h('div', {
    ...
    style: { display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'rgba(0,0,0,0.55)', zIndex: 400 },
  },
    h('div', {
      className: 'settings-pop',
      onMouseDown: (e) => e.stopPropagation(),
      style: { position: 'relative', top: 'auto', right: 'auto', width: 340, maxWidth: '90vw', animation: 'pop .15s ease' },
    }, [
      // Header
      h('div', { key: 'head', className: 'set-head' }, [
        h('span', { key: 't', className: 'sh-t' }, found ? 'Virtual Device Ready' : 'Virtual Device Setup'),
        h('span', { key: 'x', className: 'panel-collapse', onClick: onSkip }, h(UI.x)),
      ]),

      // Body
      h('div', { key: 'body', className: 'set-body' }, [
        // Description row
        h('div', { key: 'desc', className: 'set-row', style: { padding: '12px 16px' } },
          h('div', { style: { fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--text-dim)', lineHeight: 1.6 } },
            found
              ? 'Virtual audio device detected. You can now route apps through Nodus.'
              : 'Nodus needs a virtual audio device so apps can send audio through it. Choose a setup method:'
          )),
        ...
        optionCard(
          ...
          '📥',
          'VB-Audio VBCABLE',
          'Free · No restart · Works immediately',
          isVbAudio ? 'Installed ✓' : null,
          isVbAudio ? null : onInstallVbcable,
          'Download & Install',
          installing,
        ),

        // Option 2 — Nodus Driver (Test Mode)
        optionCard(
          'tm',
          '🔧',
          'Nodus Driver (Test Mode)',
          'Requires bcdedit + restart · Available when EV cert is ready',
          isNodus ? 'Active ✓' : 'Coming in release',
          null,
          null,
          true,
        ),
        ...
        // Error message
        error
          ? h('div', { key: 'err', className: 'set-row', style: { padding: '10px 16px', color: '#e07070', fontFamily: 'var(--font-mono)', fontSize: 9.5 } }, '⚠ ' + error)
          : null,
      ]),

      // Footer
      h('div', { key: 'foot', style: { padding: '10px 14px', borderTop: '1px solid var(--border)', display: 'flex', justifyContent: 'flex-end' } },
        h('button', { className: 'insp-btn', onClick: onSkip, style: { minWidth: 70 } },
          found ? 'Done' : 'Skip for now')),
    ])
  );
}
